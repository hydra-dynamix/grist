...
| CP-06-07-SECURE | §6.7 | 2 | 0 | PROVIDER | secure-message-support | **conforming** — TNEF attributes and signed/encrypted S/MIME are inert typed evidence; decryption requires an explicit runtime provider and preserves native ciphertext, provider results, identities, locators, diagnostics, and provenance separately. |
...
| CP-06-11-SUBTITLE | §6.11 | 1 | 0 | FORMAT | format-subtitles | **conforming** — bounded inert SRT, WebVTT, and TTML parsing preserves loss-bearing source facts, stable time projection, exact locators, diagnostics, and downstream graph/schema/CLI surfaces. |
...
| Email | TNEF and S/MIME | `email`; `email-message` | secure-message-support | **conforming** — structured TNEF inventory, S/MIME native/signature evidence, explicit provider routing, graph/segment behavior, generated schemas, and secret-safe failure preservation are covered. |
...
| Compression | GZIP, BZIP2, XZ, Zstandard, 7z, compound packages | formats/compression; `archives` | format-compression-7z | **conforming**: bounded stream/member decoding uses the shared recursion, cancellation, expansion, memory, member-count, provider, and archive-security controls; nested container chains, multi-member GZIP structure/header provenance, 7z timestamps/attributes, collision rejection, and deterministic member identities are preserved. |
| Image | PNG, JPEG, TIFF, WebP, GIF, BMP, HEIF/HEIC, SVG, camera metadata | formats/image; `media` | format-image-native | **conforming** — bounded native container parsing preserves dimensions, animation frames, structural chunks, associated EXIF/XMP/IPTC camera metadata, unknown payload ranges, SVG structure and inert active-content references, exact locators, graph/schema/registry/CLI surfaces, and explicit malformed, canceled, or budget-limited outcomes without decoding pixels, executing content, or fetching resources. |
| Subtitle | SRT, WebVTT, TTML, embedded subtitle tracks | formats/subtitle; `media` | format-subtitles | **conforming** — bounded inert parsing preserves source-order cues and a separate deterministic time-order transcript, strict format-specific timing, tracks versus regions, styles/settings, speakers and structured runs, decoded-text/XML/media-time locators, malformed/overlap diagnostics, and graph/segment/render/schema/registry/CLI surfaces. |
