...
use super::model::{
    WordChildArtifact, WordContentTypes, WordMacroProject, WordOoxmlOptions, WordRelationship,
    WordRelationshipTargetMode,
};
use crate::container::{
...
pub(super) fn build_artifacts(
    ...
    types: &WordContentTypes,
    relationships: &[WordRelationship],
    parent_identity: &ContentIdentity,
    options: &WordOoxmlOptions,
) -> Result<(Vec<WordMacroProject>, Vec<WordChildArtifact>), ParserError> {
    ...
    for entry in entries.iter().filter(|entry| {
        ...
        let related = relationships
            .iter()
            .filter(|relationship| {
                relationship.target_mode == WordRelationshipTargetMode::Internal
        ...
        if is_macro_part(&entry.path, content_type.as_deref(), &related) {
            ...
            .map_err(artifact_error)?;
            children.push(WordChildArtifact {
                part: entry.path.clone(),
                content_type,
                relationship_ids: relationship_ids(&related),
                locator,
...
fn is_macro_part(
    ...
    content_type.is_some_and(|value| value.eq_ignore_ascii_case(VBA_PROJECT))
        || path.eq_ignore_ascii_case("word/vbaProject.bin")
...
fn is_child_part(path: &str, relationships: &[&WordRelationship]) -> bool {
...
fn relationship_ids(relationships: &[&WordRelationship]) -> Vec<String> {
