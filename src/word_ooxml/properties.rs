...
use super::model::{
    WordCustomProperty, WordDocumentProperties, WordProperty, WordRelationship,
    WordRelationshipTargetMode,
};
use super::xml_util::{attribute, descendant_text, local_name, parse_xml_part};
...
fn property_part(relationships: &[WordRelationship], relationship_type: &str) -> Option<String> {
    relationships
        .iter()
        .find(|relationship| {
    ...
    for index in &root.children {
        ...
        if local_name(&node.name) != "property" {
            diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "word_ooxml.properties.unknown_custom_element",
            ...
            && !names.insert(name.clone())
        {
            diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "word_ooxml.properties.duplicate_custom_name",
        ...
        properties.push(WordCustomProperty {
            part: part.into(),
            name,
            property_id,
            format_id,
