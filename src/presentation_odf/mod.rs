//! Safe ODP and OTP package parser.
...
pub(crate) const PARSER: &str = "grist.presentation_odf";

pub(crate) fn parser_info() -> ParserInfo {
    ParserInfo::new(PARSER)
        .with_implementation("zip + quick-xml", "4.6.1/0.37.5")
        .with_specification_version("OASIS OpenDocument 1.2/1.3 presentation package conventions")
        .with_feature("presentation-odf")
}

pub(crate) fn parse_odp_registered(
    context: &mut ParserContext<'_>,
) -> Result<ParserOutput, ParserError> {
    parse::parse_registered(context, OdfPresentationPackageKind::Presentation)
}

pub(crate) fn parse_otp_registered(
    context: &mut ParserContext<'_>,
) -> Result<ParserOutput, ParserError> {
    parse::parse_registered(context, OdfPresentationPackageKind::Template)
}
