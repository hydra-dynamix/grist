...
use super::model::{PresentationRelationship, PresentationRelationshipTargetMode};
    ...
    for entry in entries.iter().filter(|entry| {
        ...
        let Some(source_part) = relationship_source(&entry.path) else {
            diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "presentation_ooxml.relationship.invalid_part_name",
                    format!(
            ...
            && !available.contains(source)
        {
            diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "presentation_ooxml.relationship.orphan_source",
                    format!(
        ...
        for child in &root.children {
            ...
            if local_name(&node.name) != "Relationship" {
                diagnostics.push(
                    Diagnostic::warning(
                        PARSER,
                        "presentation_ooxml.relationship.unknown_element",
            ...
            let mode = match attribute(node, "TargetMode") {
                None => PresentationRelationshipTargetMode::Internal,
                Some(value) if value.eq_ignore_ascii_case("Internal") => {
                    PresentationRelationshipTargetMode::Internal
                }
                Some(value) if value.eq_ignore_ascii_case("External") => {
                    PresentationRelationshipTargetMode::External
                }
                Some(value) => {
                    diagnostics.push(
                        Diagnostic::warning(
                            PARSER,
                            "presentation_ooxml.relationship.unknown_target_mode",
                            format!(
            ...
            let (resolved_part, target_exists) = match mode {
                PresentationRelationshipTargetMode::External => (None, None),
                PresentationRelationshipTargetMode::Internal => {
                    match resolve_internal_target(source_part.as_deref(), target) {
                        Ok(part) => {
                            ...
                            if !exists {
                                diagnostics.push(
                                    Diagnostic::warning(
                                        PARSER,
                                        "presentation_ooxml.relationship.dangling_target",
                        ...
                        Err(message) => {
                            diagnostics.push(
                                Diagnostic::warning(
                                    PARSER,
                                    "presentation_ooxml.relationship.unsafe_target",
                                    message,
                                )
                                .with_locator(locator.clone())
                                .partial(),
            ...
            if let Some(previous_part) = seen.insert(key, entry.path.clone()) {
                diagnostics.push(
                    Diagnostic::warning(
                        PARSER,
                        "presentation_ooxml.relationship.duplicate_id",
                        format!(
