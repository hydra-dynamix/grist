//! OPC content-type manifest parsing and Presentation package classification.

use super::archive::PackageEntry;
use super::model::{
    PresentationContentTypeDefault, PresentationContentTypeOverride, PresentationContentTypes,
    PresentationPackageKind,
};
...
pub(super) const PPTX_MAIN: &str =
    "application/vnd.openxmlformats-officedocument.presentationml.presentation.main+xml";
pub(super) const PPTM_MAIN: &str =
    "application/vnd.ms-powerpoint.presentation.macroEnabled.main+xml";
pub(super) const POTX_MAIN: &str =
    "application/vnd.openxmlformats-officedocument.presentationml.template.main+xml";
pub(super) const PPSX_MAIN: &str =
...
pub(super) fn parse_manifest(
    ...
    for child in &root.children {
        ...
        match local_name(&node.name) {
            "Default" => {
                ...
                defaults.push(PresentationContentTypeDefault {
            ...
            "Override" => {
                ...
                overrides.push(PresentationContentTypeOverride {
                    part_name,
                    content_type: content_type.to_string(),
                    locator: xml_locator(CONTENT_TYPES_PART, &node.path),
                });
            ...
            other => diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "presentation_ooxml.content_types.unknown_element",
    ...
    if main.len() != 1 {
        return Err(format!(
            "OOXML package must declare exactly one supported PresentationML main presentation part, found {}",
            main.len()
        ));
    }
    let (package_kind, main_presentation_part, _) = main.into_iter().next().unwrap();
    Ok(Manifest {
        content_types: PresentationContentTypes {
        ...
        main_presentation_part,
    })
}

fn diagnose_duplicates(
    defaults: &[PresentationContentTypeDefault],
    overrides: &[PresentationContentTypeOverride],
    diagnostics: &mut Vec<Diagnostic>,
) {
    for pair in defaults.windows(2) {
        if pair[0].extension == pair[1].extension {
            diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "presentation_ooxml.content_types.duplicate_default",
    ...
    for pair in overrides.windows(2) {
        if pair[0].part_name == pair[1].part_name {
            diagnostics.push(
                Diagnostic::warning(
                    PARSER,
                    "presentation_ooxml.content_types.duplicate_override",
...
pub(super) fn content_type_for(types: &PresentationContentTypes, part: &str) -> Option<String> {
...
fn package_kind(content_type: &str) -> Option<PresentationPackageKind> {
    match content_type {
        PPTX_MAIN => Some(PresentationPackageKind::Presentation),
        PPTM_MAIN => Some(PresentationPackageKind::MacroEnabledPresentation),
        POTX_MAIN => Some(PresentationPackageKind::Template),
        PPSX_MAIN => Some(PresentationPackageKind::Slideshow),
        _ => None,
    }
}

pub(super) const fn package_media_type(kind: PresentationPackageKind) -> &'static str {
    match kind {
